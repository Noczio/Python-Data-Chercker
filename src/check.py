...
from resources.concrete.check.key_finder import find_key
...
def check_arguments(*args, **kwargs) -> Generator:
    ...
    """
    try:
        min_possibilities = ("min", "low", "minimum")
        minimum = find_key(*min_possibilities, search_dict=kwargs, error_message="min, minimum or low must be provided")

        max_possibilities = ("max", "high", "maximum")
        maximum = find_key(*max_possibilities, search_dict=kwargs, error_message="max or high must be provided")

        exp_possibilities = ("expected", "type")
        expected = find_key(*exp_possibilities, search_dict=kwargs, error_message="expected or type must be provided")
